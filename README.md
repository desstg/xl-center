...
| 变量 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `AUTH_USERNAME` | 否 | 空 | 登录账号（与 `AUTH_PASSWORD` 同时设置才生效） |
| `AUTH_PASSWORD` | 否 | 空 | 登录密码 |
| `XL_DATA_DIR` | 否 | `/app/data` | 数据目录（数据库 + 缩略图） |
| `FFMPEG_PATH` | 否 | 自动 | ffmpeg 可执行文件路径 |
| `FFPROBE_PATH` | 否 | 自动 | ffprobe 可执行文件路径 |
| `LIBVA_DRIVER_NAME` | 否 | `iHD` | VAAPI 驱动名（Intel 核显用 iHD） |
...
| 路径 | 说明 |
|------|------|
| `/app/data` | 数据库（`xlcenter.db`）+ 缩略图缓存，需持久化挂载 |
| `/media` | 媒体库挂载点（示例，可自定义右侧路径，网页里填对应容器内路径） |
| `/app/config.json` | 站点配置（端口、host 等，一般无需改） |
| `/app/mapping_actor.xml` | 演员别名映射表（可选，用于演员归一化） |
