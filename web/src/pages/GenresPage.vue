...
import { getGenres, addGenre, updateGenre, deleteGenre, type TaxonomyItem } from '../api'
...
async function load() {
  ...
  try {
    ...
    const r = await getGenres(q, page.value, limit)
    genres.value = r.items
    total.value = r.total
    stats.value = r.stats
  } catch (e) {
...
function openEdit(g: TaxonomyItem) {
...
async function submit() {
  ...
  try {
    await addGenre(form.value.trim())
    showToast('类型已添加')
...
async function saveEdit() {
  ...
  try {
    await updateGenre(editingId.value, editName.value.trim())
    showToast('类型已更新')
...
async function remove(g: TaxonomyItem) {
  if (!window.confirm(`确定删除类型「${g.name}」？`)) return
  try {
...
function go(name: string) {
  router.push({ path: '/movies', query: { genres: name } })
}
...
<template>
  <div>
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.6rem">
      <h1 style="color: #666; font-size: 1.3rem">类型管理 <span style="color: #999; font-size: 0.8em; font-weight: 400">[{{ total }}]</span></h1>
      <div style="display: flex; gap: 0.5rem">
        <button class="toolbar-btn" @click="load"><RefreshCw :size="16" /> 刷新</button>
        <button class="glass-button" @click="openAdd"><Plus :size="16" /> 添加类型</button>
      </div>
    </div>
    ...
    <div style="display: flex; gap: 1rem; margin-bottom: 1rem; flex-wrap: wrap">
      <div class="glass-panel" style="flex: 1; min-width: 200px; padding: 1rem 1.2rem">
        ...
        <div style="color: var(--text-muted); font-size: 0.85rem; margin-top: 0.3rem">已关联影片的类型</div>
      </div>
      <div class="glass-panel" style="flex: 1; min-width: 200px; padding: 1rem 1.2rem">
        <div style="font-size: 1.4rem; font-weight: 700; color: var(--text-muted)">{{ stats.unused }}</div>
        <div style="color: var(--text-muted); font-size: 0.85rem; margin-top: 0.3rem">未使用类型</div>
      </div>
    </div>
    ...
    <div v-if="showForm" class="glass-panel" style="padding: 1rem; margin-bottom: 1rem; display: flex; gap: 0.7rem; align-items: center; flex-wrap: wrap">
      <input v-model="form" class="glass-input" style="flex: 1; min-width: 220px" placeholder="类型名称" @keyup.enter="submit" />
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="showEditModal = false">
      <div class="modal">
        ...
        <div style="padding: 1rem 1.2rem">
          <div style="color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 0.4rem">类型名称</div>
          <input v-model="editName" class="glass-input" style="width: 100%" placeholder="类型名称" @keyup.enter="saveEdit" />
        </div>
    ...
    <div v-else-if="genres.length" class="glass-panel" style="overflow: hidden">
      <table style="width: 100%; border-collapse: collapse; font-size: 0.88rem">
        <thead>
          <tr style="background: var(--bg); color: var(--text-secondary); text-align: left">
            ...
            <th style="padding: 0.7rem 0.9rem">类型名称</th>
        ...
        <tbody>
          <tr v-for="g in genres" :key="g.id" style="border-top: 1px solid var(--border)">
            <td style="padding: 0.6rem 0.9rem"><input type="checkbox" /></td>
            <td style="padding: 0.6rem 0.9rem"><span class="link" @click="go(g.name)">{{ g.name }}</span></td>
            <td style="padding: 0.6rem 0.9rem">{{ g.movie_count }}</td>
            <td style="padding: 0.6rem 0.9rem; color: var(--text-muted)">{{ (g.created_at || '').slice(0, 16) }}</td>
            <td style="padding: 0.6rem 0.9rem; display: flex; gap: 0.4rem">
              <button class="toolbar-btn" title="编辑" @click="openEdit(g)"><Pencil :size="14" /></button>
              <button class="toolbar-btn" title="删除" style="color: #dc2626" @click="remove(g)"><Trash2 :size="14" /></button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <p v-else-if="!loading" class="empty">暂无类型数据</p>
